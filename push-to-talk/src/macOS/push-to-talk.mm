...
void Start(const Napi::CallbackInfo &info) {
  ...
  contextData->nativeThread = std::thread([contextData] {
    ...
    auto CGEventCallback = [](CGEventTapProxy proxy, CGEventType type,
      ...
      napi_status status =
          tsfn.BlockingCall([=](Napi::Env env, Napi::Function jsCallback) {
            jsCallback.Call(
                {Napi::String::New(env, ConvertKeyCodeToString(keyCode)),
    ...
    while (contextData->shouldNativeThreadKeepRunning &&
           [theRL runMode:NSDefaultRunLoopMode
               beforeDate:[NSDate dateWithTimeInterval:0.2
...
std::string ConvertKeyCodeToText(int mac_key_code) {
  ...
  OSStatus status = UCKeyTranslate(
      keyboardLayout, static_cast<UInt16>(mac_key_code), kUCKeyActionDown,
  ...
  if (status == noErr && char_count == 0 && dead_key_state != 0) {
    isDeadKey = true;
    status = UCKeyTranslate(keyboardLayout, static_cast<UInt16>(mac_key_code),
                            kUCKeyActionDown, modifier_key_state,
